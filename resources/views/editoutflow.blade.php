...
<form action="/updateoutflow" method="post" class="mb-5">
 @csrf 
 <input type="hidden" name="id" value="{{$outflow['id']}}"  >
 <div class="form-group">
    <label>Category*</label> <span class="font-weight-bold text-danger" style="font-family:revert; font-size:77%">@error('crop_name'){{$message}}@enderror</span>
    <select name="crop_name" value="{{$outflow['crop_name']}}" class="form-control">
          <option value="" disabled>Select Product</option>
          @foreach($crops as $crop)
          <option {{$crop['name'] == $outflow['crop_name']?'selected':''}}>{{$crop['name']}}</option>
          @endforeach
         </select>
    ...
    <input name="outflow" value="{{$outflow['amount']}}" type="text" class="form-control" placeholder="Outflowed amount(kg)"> 
